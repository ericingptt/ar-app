...
選項組是**以合計面積**挑出來的，不是以成員數量：底部導覽列同樣是 2–4 個控制項，若以數量挑
會把揮動綁到導覽列而不是題目。CIBAR 的選項按鈕橫跨整個手機框、導覽項目很小，面積能可靠區分。
...
橋接腳本以 jsdom 在 CI 實測（`app/src/test/js/gesture-bridge.test.cjs`）：方向綁定、面積挑選、
`data-gesture-dir` 覆寫與「揮動確實按下對應按鈕」都有測試。
